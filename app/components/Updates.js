...
export default function Updates() {
    ...
    const fetchUpdates = async (page, search = "") => {
        try {
            ...
            if (data.length === 0) {
                ...
                setUpdates((prevTokens) => (page === 1 ? data : [...prevTokens, ...data]));
            }
    ...
    return (
        <div>
                    ...
                    {updates.map((update, index) => (
                        <div key={index} className="w-[90%] py-4">
                            <div className="flex flex-col items-start gap-3">
                                ...
                                <p className="text-[#000000] font-primary font-normal text-[12px]">Date: {new Date(update.created_at).toLocaleDateString()}</p>
                            </div>
                        </div>
                    ))}
                </>
            ...
            <div className="flex items-center gap-3 justify-between  rounded-lg w-full">
                ...
                <div className="">
                    <button className="text-[#000000] w-full px-4 py-2 bg-black text-white rounded-lg flex items-center justify-center gap-3 font-normal font-primary text-[16px]"
                        type="button"
